<template>
  <el-menu-item :index="handlePath(routeChildren.path)" @click="handleLink">
    <vab-icon
      ...
      :icon="['fas', routeChildren.meta && routeChildren.meta.icon]"
      class="vab-fas-icon"
    />
    <span>{{ routeChildren.meta && routeChildren.meta.title }}</span>
    <el-tag
      v-if="routeChildren.meta && routeChildren.meta.badge"
      type="danger"
      effect="dark"
    >
      {{ routeChildren.meta && routeChildren.meta.badge }}
    </el-tag>
  </el-menu-item>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      handleLink() {
        ...
        const target = this.routeChildren.meta && this.routeChildren.meta.target
